...
void Lexer::ParseName() {
    ...
    if (util::KeyWords.count(name) != 0) { 
        // для ключевого слова присваиваем соответсвующий токен
...
void Lexer::ParseChar() {
    ...
    if (util::DualSymbols.count(sym_pair)) {
        // если пара символов подряд - это лексема, то присваиваем
        current_token_ = util::DualSymbols.at(sym_pair);
        input_.get();
    } else { 
      // иначе берём только один символ
...
std::string ReadString(std::istream& input) {
    ...
    // последний знак - не закрывающей кавчикой -> некорректная строка
    if (c != first) {
